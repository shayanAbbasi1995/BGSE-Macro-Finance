...
delta <- 0.01 # impatience

r_safe_model <- delta + gamma * mean_congrowth - 0.5 * (gamma)^2 * var_congrowth

# not taking the last 2 values as we have NAs

beta <- (cov(real_eq_ret[1:145],realcons_growth[1:145])) / var_congrowth

risky_rate_model <- r_safe_model + (beta * gamma * var_congrowth)
...
model_results <- as.data.frame(matrix(data=NA,nrow = 1, ncol = 2))
...
data_results <- as.data.frame(matrix(data=NA,nrow = 1, ncol = 2))
